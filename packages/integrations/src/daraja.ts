...
// SAD/ADR-008 requirement: an inbound Daraja callback is not proof of payment.
// Before any order moves past PROVIDER_CONFIRMED, a server-initiated
// Transaction Status Query must confirm the payment independently.
...
// Independent verification path for the direct Daraja integration.
// This should be called before trusting a callback and before advancing the
// payment state machine beyond the provider-confirmed stage.
export async function requestTransactionStatusQuery(input: DarajaTransactionStatusInput): Promise<DarajaTransactionStatusResult> {
  const acceptedAt = new Date().toISOString();
  // Default to the live path unless the caller explicitly requests sandbox mode.
  // This is intentionally strict: direct-Daraja callbacks are not trusted as truth.
